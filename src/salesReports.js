...
class salesReports
{
  constructor(PID, PName, Pamount)
    ...
    static giveByDateB(fromDate, toDate,callback){
      ...
      products.forEach(product => {
        var salesReport1 = new salesReport(product.cartID, product.PID, product.Pname, product.Pamount, product.totalPrice, product.cDate, product.customerName, product.customerEmail);
        salesReports.push(salesReport1); 
      });
      return callback(salesReports);
      });
    ...
    static giveByDate(fromDate, toDate,callback){
      ...
      this.giveByDateB(fromDate, toDate,callback);
  //     let connection = new Connection(config);
  //   connection.on("connect", err => {
  //     if (err) {
  //       console.error(err.message);
